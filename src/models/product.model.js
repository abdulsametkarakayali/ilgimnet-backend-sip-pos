...
const product = {
  ...
  getTotalSearch: (query) => {
    return queryHelper('SELECT * FROM products WHERE name OR barcode  LIKE ?', `%${query}%`)
  },
  getAllProduct: (limit, offset, search, order, sorting) => {
    if (!order) {
      order = 'id'
    }
    const query = `SELECT products.*, categories.name as categoryName FROM products JOIN categories on products.idCategory = categories.id ${search ? `WHERE products.name OR products.barcode LIKE '%${search}%'` : ''} ORDER BY ${order} ${sorting} LIMIT ${limit} OFFSET ${offset}`
