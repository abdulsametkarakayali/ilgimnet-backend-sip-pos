...
const history = {
  getAllHistory: (order) => {
    return queryHelper(`SELECT histories.*, moneycase.*, users.name as cashier FROM histories  JOIN users ON  histories.idUser = users.id  JOIN moneycase ON histories.idUser = transacter ORDER BY date ${!order ? 'desc' : order}`)
  },
