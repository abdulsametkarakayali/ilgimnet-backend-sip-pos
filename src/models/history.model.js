...
const history = {
  getAllHistory: (order) => {
    return queryHelper(`SELECT histories.*, moneycase.*,users.name as cashier FROM histories INNER JOIN users ON  histories.idUser = users.id  INNER JOIN   moneycase histories.idUser = moneycase.transacter ORDER BY id ${!order ? 'desc' : order}`)
  },
