...
const history = {
  getAllHistory: (order) => {
    return queryHelper(`SELECT histories.*, moneycase.*, users.name as cashier FROM histories FULL JOIN users ON  histories.idUser = users.id FULL JOIN moneycase ON histories.idUser = transacter   ORDER BY id ${!order ? 'desc' : order}`)
  },
