...
const history = {
  ...
  getBestSellingToday: (id) => {
    return queryHelper('select * , sum(quantity) as salestotal  from  orderdetails o INNER JOIN histories h on o.orderDetailID  = h.id where h.paymentType = 0 and  h.paymentType =1 and   h.shiftId ='+' '+id +' '+'group by o.productID order by salestotal desc ')
  },
